...
export async function diff(repositoryPath, relativePath) {
  if (!repositoryPath) {
    throw 'The repositoryPath passed to git.diff() must not be null.';
  } else if (!await pathExists(repositoryPath)) { // don't create process with bad cwd
    throw `The repositoryPath, ${repositoryPath}, passed to git.diff() does not exist.`;
  } else if (!relativePath) {
    throw 'The relativePath passed to git.diff() must not be null.';
  } else if (!await pathExists(path.join(repositoryPath, relativePath))) { // file deleted?
    return '';
  }

  return run(repositoryPath, ['diff', gitPath(relativePath)]);
}
